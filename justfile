...
run-transcoder:
	docker run --rm -it -v ./data:/app/data {{TRANSCODER_IMAGE}}:{{DOCKER_TAG}} 

build-demo:
	docker build -t {{DEMO_IMAGE}}:{{DOCKER_TAG}} -f ./docker/Demo.Dockerfile .

run-demo:
	docker run --rm -it -v ./data:/app/data {{DEMO_IMAGE}}:{{DOCKER_TAG}} 
...
encode INFILE OUTFILE:
	/app/dependencies/mpeg-pcc-tmc2/bin/PccAppEncoder \
		...
		--config=/app/dependencies/mpeg-pcc-tmc2/cfg/sequence/redandblack_vox10.cfg \
